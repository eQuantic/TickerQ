...
{
    /// <summary>
    /// Cosmos DB implementation of the TickerQ persistence provider.
    ///
    /// IMPORTANT: Cosmos DB Limitations:
    /// - No GroupBy operations (not supported by Cosmos DB)
    /// - No complex JOINs across containers
    /// - No traditional ACID transactions across documents
    /// - Optimistic concurrency via ETags (LockHolder property)
    ///
    /// This implementation uses queries compatible with Cosmos DB's capabilities.
    /// </summary>
    internal class CosmosDbTickerPersistenceProvider<TTimeTicker, TCronTicker> :
        BasePersistenceProvider<CosmosDbTickerContext>,
        ...
        where TCronTicker : CronTicker, new()
    {
        ...
        private readonly ILogger<CosmosDbTickerPersistenceProvider<TTimeTicker, TCronTicker>> _logger;

        public CosmosDbTickerPersistenceProvider(
            CosmosDbTickerContext dbContext,
            ITickerClock clock,
            ILogger<CosmosDbTickerPersistenceProvider<TTimeTicker, TCronTicker>> logger) : base(dbContext)
        {
            _clock = clock;
            _logger = logger;
        }
            ...
            Action<TickerProviderOptions> options = null, CancellationToken cancellationToken = default)
        {
            ...
            // Cosmos DB doesn't support traditional transactions like SQL
            // Using optimistic concurrency via ETag (LockHolder as concurrency token)
            var timeTickerContext = GetDbSet<TimeTickerEntity>();
